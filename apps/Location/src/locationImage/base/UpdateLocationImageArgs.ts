/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { LocationImageWhereUniqueInput } from "./LocationImageWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { LocationImageUpdateInput } from "./LocationImageUpdateInput";

@ArgsType()
class UpdateLocationImageArgs {
  @ApiProperty({
    required: true,
    type: () => LocationImageWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => LocationImageWhereUniqueInput)
  @Field(() => LocationImageWhereUniqueInput, { nullable: false })
  where!: LocationImageWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => LocationImageUpdateInput,
  })
  @ValidateNested()
  @Type(() => LocationImageUpdateInput)
  @Field(() => LocationImageUpdateInput, { nullable: false })
  data!: LocationImageUpdateInput;
}

export { UpdateLocationImageArgs as UpdateLocationImageArgs };
